...
$(document).ready(function(){
	...
	function init() {
		var mapOptions = {
			zoom: 3,
			center: new google.maps.LatLng(53.344507, -6.247502),
		...
		var contentString = "64 Westland Square, Pearse St, Dublin 2";
	...
	$.validate({
		...
		onSuccess:function(){
			...
			$('p.error').html('Sending your message...');
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					if(data.send){
					  $('.sendcontact').prop("disabled",true);
					  $('p.error').html('Your message was sent successfully.');
					}else{
						$('p.error').html("Sorry! Message not sent, try later.");
					}
				},
				error: function(e){
					console.log('ERROR');
